...
a = np.array([0.0, 0.0, 0.0], dtype=np.float32)  # [steer, gas, brake]
filename = "all_episodes.npz"
all_frames = []
all_actions = []
...
# 🔄 Global persistent memory variables — define once outside the function:
steer_memory = 0.0
gas_memory = 0.0
brake_memory = 0.0

# 🔄 Global memory variables (declare once globally)
steer_memory = 0.0
gas_memory = 0.0
brake_memory = 0.0
...
if os.path.exists(filename):
    ...
    try:
        ...
        print(f"Loaded {frame_number} frames.")
    except Exception as e:
        print(f"Failed to load: {e}")

while not quit and frame_number < 15000:
    env.reset()
    total_reward = 0.0
    steps = 0
    restart = False
    ...
    while True:
        ...
        if frame_number >= 15000 or terminated or truncated or restart or quit:
            break
